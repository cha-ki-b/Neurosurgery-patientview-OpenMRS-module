    ...
    <div class="neuro-content">
        ...
        <div class="neuro-panel">
            ...
            <div class="neuro-panel-content">
                ...
                <% } else { %>
                <div class="neuro-record-item">
                    <div><strong>Diab&egrave;te :</strong> ${ medicalHistory.diabetes ? "Oui" : "Non" }</div>
                    <div><strong>Hypertension :</strong> ${ medicalHistory.hypertension ? "Oui" : "Non" }</div>
                    <div><strong>&Eacute;pilepsie :</strong> ${ medicalHistory.epilepsy ? "Oui" : "Non" }</div>
                    <div><strong>AVC :</strong> ${ medicalHistory.stroke ? "Oui" : "Non" }</div>
                    <div><strong>Cardiopathie :</strong> ${ medicalHistory.heartDisease ? "Oui" : "Non" }</div>
                    <div><strong>Insuffisance r&eacute;nale :</strong> ${ medicalHistory.renalFailure ? "Oui" : "Non" }</div>
                    <% if (medicalHistory.allergies) { %><div><strong>Allergies :</strong> ${ ui.format(medicalHistory.allergies) }</div><% } %>
                    <% if (medicalHistory.chronicTreatment) { %><div><strong>Traitement chronique :</strong> ${ ui.format(medicalHistory.chronicTreatment) }</div><% } %>
                    <% if (medicalHistory.otherHistory) { %><div><strong>Autres :</strong> ${ ui.format(medicalHistory.otherHistory) }</div><% } %>
                    <% if (medicalHistory.dateChanged) { %>
                        <span class="neuro-assessment-date">Derni&egrave;re mise &agrave; jour : ${ ui.format(medicalHistory.dateChanged) }</span>
                    <% } %>
                </div>
